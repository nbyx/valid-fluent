...
describe("e2e-library-test", () => {
	...
	test("should validate UserModel password field", () => {
		...
		if (!outcome.isValid) {
			...
			expect(outcome.result.password.message).toContain(
				"Password must be a UUID",
			);
		}
	});
	...
	test("should validate UserModel active field", () => {
		...
		if (!outcome.isValid) {
			...
			expect(outcome.result.active.message).toContain(
	...
	test("should validate UserModel birthDate field", () => {
		...
		if (!outcome.isValid) {
			...
			expect(outcome.result.birthDate.message).toContain(
