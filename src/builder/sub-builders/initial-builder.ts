...
export class InitialBuilder<ModelType, FieldType = unknown, DependentFieldType = unknown> {
    constructor(
        protected readonly failFast: boolean = true,
        protected readonly validationRules: ReadonlyArray<ValidationRule<ModelType, FieldType, DependentFieldType>> = [],
    ) {}
    ...
    forField<NewFieldType>(
        ...
        const newValidationRules = [...this.validationRules, newRule] as unknown as ValidationRule<
            ModelType,
            NewFieldType,
            DependentFieldType
        >[];

        return new ForFieldAddedBuilder<ModelType, NewFieldType, DependentFieldType>(this.failFast, newValidationRules);
    }
}