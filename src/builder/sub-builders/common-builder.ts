...
export class CommonBuilder<ModelType, FieldType, DependentFieldType> {
    ...
    forField<NewFieldType>(
        ...
        new InitialBuilder(this.failFast, this.validationRules).forField(name, propGetter);

        return new ForFieldAddedBuilder<ModelType, NewFieldType, DependentFieldType>();
    }
    ...
    when<Field, DependentField>(
        ...
        if (builderCallback) {
            ...
            const newValidationRules = [...this.validationRules, ...subValidationRules] as ValidationRule<
                ModelType,
                FieldType,
                DependentFieldType
            >[];

            return new CommonBuilder<ModelType, FieldType, DependentFieldType>(this.failFast, newValidationRules);
        }

        if (this.validationRules.length === 0) throw new Error("Call 'forProperty' before using 'when'");

        const currentRule = this.validationRules[this.validationRules.length - 1];
        if (!currentRule) throw new Error("A precondition is not met: 'currentRule' is not set.");

        const currentValidator = currentRule.validators[currentRule.validators.length - 1];
        if (!currentValidator) throw new Error("A precondition is not met: 'currentValidator' is not set.");
    ...
    build(): Validation<ModelType> {
        return new Validation(this.failFast, [
            ...(this.validationRules as ValidationRule<ModelType, unknown, unknown>[]),
        ]);
    }
}