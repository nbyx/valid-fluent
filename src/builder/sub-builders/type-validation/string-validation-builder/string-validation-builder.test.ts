...
describe("StringValidationBuilder", () => {
	...
	test("should validate maxLength", () => {
		...
		if (!outcome.isValid)
			expect(outcome.result.username.message).toBe(
	...
	test("should validate isUUID", () => {
		...
		const validation = builder.build();
		const model = {
			id: "0bd7f2f9-aa70-4a35-9a44-2472f0d77d90",
			username: "john_doe123",
			email: "",
			password: "",
			website: "",
	...
	test("should validate isIn", () => {
		...
		if (!outcome.isValid)
			expect(outcome.result.password.message).toBe(
				"Favorite fruit must be one of the specified values",
			);
	});
	...
	test("should validate isURL", () => {
		...
		expect(outcome.isValid).toBe(false);
		if (!outcome.isValid)
			expect(outcome.result.website.message).toBe(
